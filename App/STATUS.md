...
| Efekt przy notchu | Ustawienia → Wygląd: Brak / Łapka (domyślnie). **Podświetlenie usunięte całkowicie** (David: „działało niebo lepiej wcześniej"; cały kod koloru/siły/gradientu/ambientnej poświaty panelu wyrzucony, nie tylko schowany). Stare zapisy z "glow" ładują się jako Łapka. Łapka: SZTYWNE ramię (ok. 15% giętkości = lekkie sprężyste dobieganie), łokieć tuż przy notchu, przedramię i pęk pięciu palców skierowane w stronę kursora, grubość 31→25 pt przy notchu 220 pt; reaguje TYLKO na kursor w promieniu ok. 105 pt (wysuwanie/chowanie wolne, chowanie z opóźnieniem 0,45 s; bark przeskakuje krokami po notchu, dłoń „pacuje” w cyklu ok. 1,15 s); na bocznych krawędziach działa tylko podświetlenie | testy (IK, sprężyna), renderowanie póz, test w prawdziwym oknie z symulowanym kursorem; **ruch na żywo niesprawdzony** |
