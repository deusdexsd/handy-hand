...
| Widok minimalistyczny | ikonka w toolbarze (obok tagu) + Ustawienia → Wygląd: chowa nazwę i wymiary/długość pod kaflem; kafle obrazów/wideo mają teraz proporcje materiału (pion zostaje pionem, poziom poziomem, 70–260 pt), nie sztywny prostokąt — bliżej Pinteresta. Wymaga wymiarów w indeksie: obrazy miały je zawsze, wideo dostało je dopiero teraz (`AVAssetTrack.naturalSize` + transform) — **stare, już zaindeksowane pliki wideo dostaną wymiary dopiero przy ponownym zaindeksowaniu**, nie wstecznie | render (siatka ze zróżnicowanymi wysokościami potwierdzona po świeżym indeksowaniu) |
