...
| Źródła | foldery i biblioteki FCP (czytane jak folder, tylko „Original Media”), symlinki rozwiązywane, obserwowanie zmian na żywo | 60 testów + autotest (plik dodany i usunięty widać w indeksie) |
...
| Ulubione | gwiazdka na kaflu i w liście jest klikalna (dodaje/usuwa bez menu); włącznik „Ulubione zawsze na górze listy” w Ustawieniach → Ogólne | test sortowania (rdzeń) + render |
...
swift test --scratch-path ~/Library/Caches/MidniteDockBuild-App   # 60 testów
```
