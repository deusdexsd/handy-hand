...
| Efekt przy notchu | Ustawienia → Wygląd: Brak / Podświetlenie (kolor z kółka kolorów albo 3 gotowe, suwak siły 0,3–2) / Łapka (domyślnie). Statyczna ikonka łapki w pigułce notcha usunięta. Podświetlenie wylewa się od dołu na zewnątrz pigułki/notcha (dwuwarstwowy gradient poza kształtem, nie przycięty do środka jak wcześniej — dotyczy i prawdziwego notcha, i wirtualnej wysepki); w zasięgu skacze do pełnej jasności i tam ZOSTAJE (histereza), zamiast śledzić dystans na bieżąco; przy rozwiniętym panelu i aktywnym Podświetleniu zostaje stała, spokojna poświata u góry panelu. Łapka: SZTYWNE ramię (ok. 15% giętkości = lekkie sprężyste dobieganie), łokieć tuż przy notchu, przedramię i pęk pięciu palców skierowane w stronę kursora, grubość 31→25 pt przy notchu 220 pt; reaguje TYLKO na kursor w promieniu ok. 105 pt (wysuwanie/chowanie wolne, chowanie z opóźnieniem 0,45 s; bark przeskakuje krokami po notchu, dłoń „pacuje” w cyklu ok. 1,15 s); na bocznych krawędziach działa tylko podświetlenie | testy (IK, sprężyna), renderowanie póz, test w prawdziwym oknie z symulowanym kursorem; **ruch na żywo niesprawdzony** |
...
swift test --scratch-path ~/Library/Caches/MidniteDockBuild-App   # 61 testów
```
