...
final class IndexerTests: XCTestCase {
    ...
    func testSidebarHiddenAndMinimalistGridDefaultsAndDecode() throws {
        ...
        XCTAssertEqual(old.panelTransparency, 0.5); XCTAssertEqual(old.thumbnailQuality, .normal); XCTAssertEqual(ThumbnailQuality.low.maxPixel, 160)
        XCTAssertEqual(try JSONDecoder().decode(AppSettings.self, from: Data("{\"panelTransparency\":7}".utf8)).panelTransparency, 1)
